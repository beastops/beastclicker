...
public sealed class ClickEngine : IDisposable
{
    ...
    /// </summary>
    private void SleepPrecise(double ms)
    {
        if (_timer != IntPtr.Zero)
        {
            ...
            {
                WaitForSingleObject(_timer, (uint)(ms + 50));
                return;
            }
        }
    ...
    private bool WaitUntil(Stopwatch sw, long deadline)
    {
        while (true)
        {
            ...
            if (remain > _spinTicks)
            {
                // Cap each wait so a stop request is still noticed promptly.
                double sleepMs = Math.Min((remain - _spinTicks) / TicksPerMs, 30.0);
                SleepPrecise(sleepMs);
            }
            else
            {
    ...
    private void Loop(EngineSettings s)
    {
        ...
        _spinTicks = (long)((s.IntervalMs < 3 ? 0.45 : 0.25) * TicksPerMs);
        ...
        try
        {
            ...
            while (_running)
            {
                ...
                if (!WaitUntil(sw, deadline)) break;
                ...
                if (!s.DryRun)
                {
                    if (discreteHold)
                    {
                        ...
                        WaitUntil(sw, sw.ElapsedTicks + holdTicks);
